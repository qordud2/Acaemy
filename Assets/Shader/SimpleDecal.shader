Shader "Custom/SimpleDecal"
{
		...
		SubShader
		{
			Tags { "RenderType" = "Transparent" "Queue" = "Transparent" }	// Transparent - 알파값이 적용된다
			Pass
			{
				ZWrite Off
				Lighting Off
				...
				float depth = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.screenPos.xy / i.screenPos.w));	// w - depth, w로 나눠야 원래 좌표값이 나온다
		...
		FallBack "Unlit/Transparent"	// FallBack 여러가지 이유로 실행이 불가능할 경우 "Unlit/Transparent" 이것이 실행된다는 뜻
}